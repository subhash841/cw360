...
var articleid = $("body").attr("data-detailid");
...
$(function () {
    ...
    $("a.head-login").attr("href", base_url + "Login?section=article");

    var default_article_count = 7;

    var article_list_offset = 0;
    load_rated_articles(article_list_offset);
    ...
    if (articleid != "") {
        var latest_article_list_offset = 0;
        load_latest_voices(latest_article_list_offset);
        $("a.head-login").attr("href", base_url + "Login?section=voicedetail&vid=" + articleid);
    }

    $(".blog-list-page .load-more-voice-list").on("click", function (e) {
        if (article_list_offset == 0) {
            default_article_count = 7;
        } else {
            default_article_count = 6;
        }
        article_list_offset = article_list_offset + default_article_count;
        load_rated_articles(article_list_offset);
    });
    ...
    $(".btn_create_voice").on("click", function (e) {
        ...
        var body = '';
        if (uid == 0) {
            window.location.assign(base_url + "Login?section=article");
    ...
    $('#silverPointsCheck button#create_voice_yes').on("click", function () {
        window.location.assign(base_url + "RatedArticle/raise_article");
    });
    if (options.toast) {
...
//    });
    ...
    $(".post-comment").on("click", function (e) {
        e.preventDefault();
        if (uid == 0) {
            window.location.assign(base_url + "Login?section=voicedetail&vid=" + articleid);
            return false;
        }
        var _this = $(this);
...
function load_rated_articles(offset) {
    $.ajax({
        url: base_url + "RatedArticle/lists",
        method: "POST",
...
function getblock(data, top = false) {
    ...
    var a = $("<a/>", {
        href: base_url + 'RatedArticle/details/' + data.id + '/' + slug + '?t=' + timestamp,
        class: "d-block pb-3",
        "data-content": "Most Popular Article",
...
var load_latest_voices = function (offset) {
    ...
    $.ajax({
        ...
        method: "POST",
        data: {offset: offset, articleid: articleid}
    }).done(function (result) {
        result = JSON.parse(result);
