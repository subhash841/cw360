...
class Predictions extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->load->model(array('games_model', 'predictions_model','sidebar_model'));
...
//        $data['sidebar_blogs'] = $this->blog_model->get_blog_by_topic_list(4,$data['game_details']['topic_id']);
        $data['sidebar_blogs'] = $this->sidebar_model->get_all_blogs(sidebar_card_limit(),$data['game_details']['topic_id'],0);
        $data['sidebar_quiz'] = $this->sidebar_model->get_all_quiz(sidebar_card_limit(),$data['game_details']['topic_id'],0);
        $data['play_game_cookie'] = get_cookie('game_'.$game_id.'_play');
        // print($data['play_game_cookie']);die;
        // delete_cookie('game_'.$game_id.'_play');die;
        // $data['user_game_joureny'] = false;
        // if(!isset($_COOKIE['game_user_journey']) && !empty( $allpredictionDetails) ){
        //   setcookie('game_user_journey', true, time()+31556926 );
        //   $data['user_game_joureny'] = true;
        // }
...
//        $this->load->view('predictions', $data);
  ...
  function prediction_action() {
        ...
        $game_available = $this->games_model->check_game_existance($postData['game_id']);
        if (empty($this->user_id) || (!empty($this->user_id) && empty($play_game_cookie))) {
          ...
          $check_prediction_swiped = $this->predictions_model->check_prediction_swiped($this->user_id,$postData['game_id'],$postData['current_prediction_id']);
          ...
          if ($current_prediction_data==false || empty($current_prediction_data)) {
            $data = array('status' => 'failure','message' => 'Prediction not found','errorShow'=>'console','reload'=>'no');
              ...
              $data = array('status' => 'failure','message' => 'Prediction has been already swiped','errorShow'=>'modal','reload'=>'yes');
          }else if($game_points < $current_prediction_price && $postData['swipe_type']=='right') {
              $data = array('status' => 'failure','message' => 'Insufficient game coins','errorShow'=>'modal','reload'=>'no');
              ...
              $data = array('status' => 'failure','message' => 'This prediction is not available now','errorShow'=>'modal','reload'=>'yes');
          }else{
              ...
              $load_prediction_data = array('load_prediction_data' => $result_load_prediction);
                if (!empty($result_load_prediction)) {
                  ...
                  $data = array('status' => 'failure','message' => 'prediction not inserted or game coins not updated','errorShow' => 'console');
              }
          }
    ...
    function deduct_coins() {
        $postData = $this->input->post();
        $check_user_points = $this->predictions_model->get_game_points($postData['game_id'],$this->user_id);
        if (!empty($check_user_points)) {
          $data = array('status' => 'failure', 'message' => 'Coins have been already added to play this game', 'type' => 'points_exist');
        }else{
            ...
            if (!empty($user_coins)) {
                if ($user_coins['coins'] < $postData['req_game_points']) {
                  ...
                  $game_details = $this->games_model->get_game_details($postData['game_id']);
                  ...
                  //$fixed_points = 1000;
                    ...
                    if ($add_game_points == 'coins_query_failed') {
                        ...
                        $data = array('status' => 'failure', 'message' => 'Coins not inserted','type' => 'query_issue');
                    } elseif ($add_game_points == 'points_log_query_failed') {
                        $data = array('status' => 'failure', 'message' => 'Coins log not inserted','type' => 'query_issue');
                    } else {
                        $data = array('status' => 'success', 'message' => $fixed_points . ' Coins have been added to play this game');
                    }
                }
            } else {
                $data = array('status' => 'failure', 'message' => 'You do not have enough coins in your wallet', 'type' => 'not_enough_coins');
    ...
    function leaderboard($game_id = '') {
        ...
        //   $data['user_leaderboard_joureny'] = false;
        // if(!isset($_COOKIE['leaderboard_user_journey'])){
        //   setcookie('leaderboard_user_journey','true',time()+31556926 );
        //   $data['user_leaderboard_joureny'] = true;
        // }
...
//        $this->load->view('predictions', $data);
    ...
    function summary($game_id = '') {
        ...
        // $data['user_summary_joureny'] = false;
        // if(!isset($_COOKIE['summary_user_journey'])){
        //   setcookie('summary_user_journey','true',time()+31556926 );
        //   $data['user_summary_joureny'] = true;
        // }
        $this->load->view('header',$data);
        $this->load->view('main_game', $data);
//        $this->load->view('predictions', $data);
                ...
                if ($user_coins['coins'] < $postData['req_game_points']) {
                  ...
                  //$fixed_points = 1000;
                    ...
                    if ($add_game_points == 'coins_query_failed') {
                      ...
                      if (empty(get_cookie('game_'.$postData['game_id'].'_play'))) {
                        $rewards_cookie = array(
                          'name'   => 'game_'.$postData['game_id'].'_play',
                          'value'  => 'play_game_'.$postData['game_id'],
                          'expire' => 86400 * 730
                        );
                        set_cookie($rewards_cookie);
                      }
